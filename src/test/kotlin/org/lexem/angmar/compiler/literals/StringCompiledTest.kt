...
internal class StringCompiledTest {
    @Test
    ...
    @Test
    fun `test with escaped expression`() {
        val text =
                "${StringNode.startToken}${EscapedExpressionNode.startToken}12${EscapedExpressionNode.endToken}abc${StringNode.endToken}"
        val analyzer = TestUtils.createAnalyzerFrom(text, parserFunction = StringNode.Companion::parse)
        TestUtils.processAndCheckEmpty(analyzer)

        val result = analyzer.memory.getLastFromStack() as? LxmString ?: throw Error("The result must be a LxmString")
        Assertions.assertEquals("12abc", result.primitive, "The primitive property is incorrect")
